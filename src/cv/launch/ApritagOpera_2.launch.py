...
# 使用示例：
# 真实摄像头 + 真实串口（需指定串口设备）
# ros2 launch cv ApritagOpera_2.launch.py use_sim:=false serial_sim:=false port:=/dev/ttyUSB0
# 真实摄像头 + 串口模拟（无物理串口）
...
# 默认值在同文件的 DEFAULT_SIZE / DEFAULT_FPS；tag 的边长/ID/内参文件归 apriltag
# 模块（AprilTagParams，标定文件它自己找），所以这里只留 use_sim / frame_id / 串口。
...
def generate_launch_description():
    ...
    return LaunchDescription([
          ...
          # pose_method 不写 = 用 apriltag 模块的默认（pnp 全位姿：卡尔曼 + TF）；
          # 本文件要的就是默认那套，所以不覆盖 —— 默认值只该有一个出处。
