...
# 使用示例：
# 真实摄像头 + 真实串口（需指定串口设备）
# ros2 launch cv cam_pos.launch.py use_sim:=false serial_sim:=false port:=/dev/ttyUSB0 camera_id:=4
# 真实摄像头 + 串口模拟（无物理串口）
# ros2 launch cv cam_pos.launch.py use_sim:=false serial_sim:=true
# 模拟图像（合成 tag）+ 串口模拟
# ros2 launch cv cam_pos.launch.py use_sim:=true serial_sim:=true
